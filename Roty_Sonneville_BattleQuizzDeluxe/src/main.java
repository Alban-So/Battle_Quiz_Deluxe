...
//#region-------------------------------------------CONSTRUCTEUR ET CONTROLES DE SAISIE-----------------------------------------------------//
    ...
    Joueur formulaireCréationJoueur(int numJoueur){
        ...
        do{
            ...
            if(equals(pseudo,"BOT")){
                cursor(22,16);
                println("Ce pseudo est réservé");
            }
        }while(equals(pseudo,"") || equals(pseudo,"BOT"));
    ...
    Carte definitCarte(int indiceCarte){
        ...
        CSVFile listeCartes = loadCSV("./ressources/fichiersCSV/cartes.csv");
...
//#region-----------------------------------------FONCTIONS D'INTERACTION AVEC LES CARTES---------------------------------------------------//
    boolean placerCarteMort(Carte[][] plateauDeCartes, Joueur joueurQuiJoue, String[]historiqueJeux,boolean cestJ1, String carteVide){
        String menuChoixMort=TXTtoString(newFile("./ressources/Texts/InteractionMort.txt"));
        afficherHistoriquePartie(historiqueJeux,"Votre monstre a été éliminé, veuillez placer une carte du BANC sur zone ACTIF");
        cursor(2,0);
        print(menuChoixMort);
    ...
    boolean echangerCarte(Carte[][] plateauDeCartes, Joueur joueurQuiJoue, String[] historiqueJeux,boolean cestJ1,String carteVide){
        String menuChoixEchange=TXTtoString(newFile("./ressources/Texts/InteractionEchange.txt"));
    ...
    void placerCarte(Carte[][] plateauDeCartes,Joueur joueurQuiJoue,String[]historiqueJeux){
        ...
        String affichageSoinChoix=TXTtoString(newFile("./ressources/Texts/MenuCarteSoin.txt"));
    ...
    void placerMonstreBot(Carte[][] plateauDeCartes,Joueur joueurQuiJoue,String[] historiqueJeux){
        ...
        if (plateauDeCartes[joueurQuiJoue.indiceLignePlateauDeCarte][0] == null) {
            while (i < compteurCarteMain(joueurQuiJoue.main)) {
                if (equals(joueurQuiJoue.main[i].type, "Monstre")) {
                    placerCarteAlgo(plateauDeCartes, joueurQuiJoue, i + 1, 0, historiqueJeux);
                    break;
                }
                i++;
            }
        ...
        for (int j = 1; j < 3; j++) {
            i = 0;
            while (i < compteurCarteMain(joueurQuiJoue.main)) {
                if (equals(joueurQuiJoue.main[i].type, "Monstre") && plateauDeCartes[joueurQuiJoue.indiceLignePlateauDeCarte][j] == null) {
    ...
    boolean aUnMonstre(Carte[] mainJoueur){
       boolean res=false;
        for(int i=0;i<compteurCarteMain(mainJoueur);i++){
            if (equals(mainJoueur[i].type,"Monstre")){
                res=true;
            }
        }
        return res; 
...
//#region---------------------------------------------------FONCTION D'AFFICHAGE------------------------------------------------------------//
    ...
    void afficherPiece(int tirage){
        ...
        File[] fichiersPieces = { // Fichiers texte pour l'animation de la pièce
            new File("./ressources/Texts/Pieces/pieceEuroPlat1.txt"),
            new File("./ressources/Texts/Pieces/pieceEuroPlat0.txt"),
            new File("./ressources/Texts/Pieces/pieceEuroPlat2.txt")
        };
        for (int i=0; i<3; i++){
            cursor(10, 0);
        ...
        if(tirage == 1){ // Si on a tiré face
            cotéGagnant = newFile("./ressources/Texts/Pieces/pieceFace.txt");
        }
        else{            // Si on a tiré pile
            cotéGagnant = newFile("./ressources/Texts/Pieces/piecePile.txt");
        }
...
//Cette fonction execute toute les précédentes et indique à l'algorithme principale si l'attaque est validé par le biais d'un qcm qui s'adapte à la classe du joueur
    boolean QCMpourAttaquer(Joueur j,int choixDifficulté,String[] historique){
        ...
        CSVFile cp = loadCSV("./ressources/fichiersCSV/Questions/questionCP.csv");
        CSVFile ce1 = loadCSV("./ressources/fichiersCSV//Questions/questionCE1.csv");
        CSVFile ce2 = loadCSV("./ressources/fichiersCSV//Questions/questionCE2.csv");
        CSVFile cm1 = loadCSV("./ressources/fichiersCSV//Questions/questionCM1.csv");
        CSVFile cm2 = loadCSV("./ressources/fichiersCSV//Questions/questionCM2.csv");
        CSVFile sixième = loadCSV("./ressources/fichiersCSV//Questions/question6e.csv");
        //-------------------------------------------//
...
//#region------------------------- Fonctions de sauvegarde --------------------------------------------------------------------------//
    ...
    String nomPartieSave = "./ressources/fichiersCSV/partieSave.csv";
    ...
    void sauvegarderPlateau(Carte[][] plateau){
        ...
        String nomPlateauSave = "./ressources/fichiersCSV/plateauSave.csv";
    ...
    void sauvegarderInfosJoueurs(Joueur joueur1, Joueur joueur2){
        ...
        String nomInfosJoueursSave = "./ressources/fichiersCSV/infosJoueursSave.csv";
    ...
    void ajoutCarteSauvegardeeDansPlateau(Carte[][] plateau){
        ...
        CSVFile partieSave = loadCSV("./ressources/fichiersCSV/plateauSave.csv");
...
// ------------------------- Fonctions de test ------------------------- //
    ...
    void testdeclarationDeck(){
        CSVFile CSV= loadCSV("./ressources/fichiersCSV/partieSave.csv");
...
//#region---------------------------------------------------BOUCLES DU JEU------------------------------------------------------------------//
    boolean choixJoueurPartie1vBOT(Carte[][] plateauDeCartes, Joueur joueurQuiJoue,String[]historiqueJeux,boolean cestJ1,String carteVide,String logoQCMString,String menuMainJoueur,Joueur j1, Joueur j2,int numTour,int joueurQuiCommence){
    ...
    String menuJeux(){
	    ...
	    File TxtBattleQuiz = newFile("./ressources/Texts/BattleQuizDeluxe.txt"); //Stockage du contenu des fichiers texte dans une variable
        File TxtAccueil = newFile("./ressources/Texts/accueil.txt");
        ...
        do{ // L'utilisateur choisi un numéro entre 1 et 6 (inclus)
            cursor(20, 2);
            print("Entrez un numéro entre 1 et 6 pour valider :");
            choixUtilisateur = readString();
            if (length(choixUtilisateur) == 0);
    ...
    void partieEnCours(CSVFile fichierSauvegarde, String typeDePartie,boolean partieSauvegardee){
        //---------------------------INITIALISATION COMMUNE AUX 2 TYPES DE PARTIES--------------------------------------------------------//
            ...
            //-------Affichage générer----------//
            String BorduresJeux = TXTtoString(newFile("./ressources/Texts/BorduresJeux.txt"));
            String logoQCMString=TXTtoString(newFile("./ressources/Texts/EncadréQuestion.txt"));
            String menuMainJoueur=TXTtoString(newFile("./ressources/Texts/InteractionMain.txt"));
            String stylePlateau=TXTtoString(newFile("./ressources/Texts/Plateau.txt"));
            String carteVide=TXTtoString(newFile("./ressources/Texts/carteVide.txt"));
            //#endregion
            cursor(0, 0);
            print(BorduresJeux);
            cursor(2,0);
            ...
            if (partieSauvegardee){ // Chargement des Joueurs avec leur classe
                j1.pseudo = getCell(loadCSV("./ressources/fichiersCSV/infosJoueursSave.csv"), 1, 0); 
                j1.classe = getCell(loadCSV("./ressources/fichiersCSV/infosJoueursSave.csv"), 1, 1); 
                viderPlateau();
                j2.pseudo = getCell(loadCSV("./ressources/fichiersCSV/infosJoueursSave.csv"), 2, 0);
                j2.classe = getCell(loadCSV("./ressources/fichiersCSV/infosJoueursSave.csv"), 2, 1); 
                viderPlateau();
                cursor(9, 105);
                    ...
                    if(numTour==0 || numTour==1){
                        ...
                        if(plateauDeCartes[joueurQuiJoue.indiceLignePlateauDeCarte][0]==null){
                            if(plateauDeCartes[joueurQuiJoue.indiceLignePlateauDeCarte][1]!=null || plateauDeCartes[joueurQuiJoue.indiceLignePlateauDeCarte][2]!=null){
                                finDePartie=placerCarteMort(plateauDeCartes,joueurQuiJoue,historiqueJeux,cestJ1,carteVide);
                                return;
                            }else{
                        ...
                        if(equals(typeDePartie,"1vs1")){
                            ...
                            finDePartie=choixJoueurPartie1vBOT(plateauDeCartes,joueurQuiJoue,historiqueJeux,cestJ1,carteVide,logoQCMString,menuMainJoueur,j1,j2,numTour,joueurQuiCommence);
                        }
                    }
                numTour++;
    ...
    void algorithm() {
        ...
        do{ // Tant que le joueur ne quitte pas le jeu, il reste dans le menu // test de la boucle while
            File BorduresJeux = newFile("./ressources/Texts/BorduresJeux.txt");
            cursor(0, 0);
            ...
            switch (choixJoueur){
                case "1": // Choix d'une partie solo contre bot
                    partieSélectionnée = loadCSV("./ressources/fichiersCSV/partieVide.csv");
                ...
                case "3": // Choix d'une partie deja sauvegardee
                    partieSélectionnée = loadCSV("./ressources/fichiersCSV/partieSave.csv");
                    partieEnCours(partieSélectionnée,"1v1", true);// On lance la partie;
                    break;

                case "4": // Afficher les règles du jeux 
                    print("Le principe est qu'à travers un combat de carte, vous obteniez 3 points ou que vous éliminiez le monstre adverse\n sans qu'il ait de monstre sur son banc");
                    break;      
                case "5": // Crédit du jeu
                    print("Jeu conçu par Clément Roty et Alban Sonneville en BUT 1 Informatique de Lille");
                    break;

                case "6": // Choix de fin du jeu
                    finJeu = true;
                    break;
                default:
                    print("\n| Erreur");
